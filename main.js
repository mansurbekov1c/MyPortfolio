...
window.addEventListener('scroll',()=>{
  let cur='';
  allSections.forEach(s=>{if(s.id&&window.scrollY>=s.offsetTop-100)cur=s.id;});
...
async function sendForm(){
  ...
  */
  const TOKEN = 'YOUR_BOT_TOKEN'; // <-- shu yerga bot tokeningizni qo'ying
  const CHAT_ID = 'YOUR_CHAT_ID'; // <-- shu yerga chat id ingizni qo'ying
